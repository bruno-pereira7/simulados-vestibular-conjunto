  ...
  return (
    <div className="min-h-screen bg-gray-100 px-6 py-10">
      <div className="mx-auto max-w-4xl rounded-xl bg-white p-8 shadow-md">
        <h1 className="mb-6 text-3xl font-bold text-gray-800">Perfil do Usuário</h1>
        ...
        <div className="flex flex-col items-center gap-6 md:flex-row">
          <div className="relative">
            ...
            />
            <label className="absolute bottom-0 right-0 cursor-pointer rounded-full bg-blue-600 px-2 py-1 text-sm text-white hover:bg-blue-700">
              <input
                type="file"
                accept="image/*"
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
          <div className="rounded-lg bg-blue-50 p-4 shadow">
            <h3 className="mb-2 text-lg font-semibold text-blue-700">Histórico de Vestibulares</h3>
            <ul className="space-y-1 text-gray-700">
              {historicoVestibulares.map((pontos, idx) => (
                <li key={idx}>Prova {idx + 1}: {pontos} acertos</li>
              ))}
            </ul>
          </div>

          <div className="rounded-lg bg-green-50 p-4 shadow">
            <h3 className="mb-2 text-lg font-semibold text-green-700">Pontuação em Redações</h3>
            <ul className="space-y-1 text-gray-700">
              {historicoRedacoes.map((nota, idx) => (
                <li key={idx}>Redação {idx + 1}: {nota} pontos</li>
              ))}
            </ul>
          </div>

          <div className="rounded-lg bg-yellow-50 p-4 shadow">
            <h3 className="mb-2 text-lg font-semibold text-yellow-700">Recorde Pessoal</h3>
            <p className="text-gray-700">Acertos seguidos: <span className="font-bold">{recordeSeguido}</span></p>
          </div>
        </div>
      </div>
    </div>
